...
Key Messages:
\begin{compactenum}
\item A method to infer the true Sharpe ratio of the Markowitz portfolio is given.
...
# one sentence only:
fig_cap <- paste0("The empirical type I rate, over ",prty(nrep)," simulations, of a one-sided confidence bound for $\\psnr_{a}$ are shown ",
                  "for a nominal type I rate of $",typeI,"$, for multivariate normal returns. ")
...
ph <- nures %>%
  ...
  labs(x=expression(n/k),
       ...
       caption='The y axis is in square root scale to show detail.')
print(ph)
@
%\end{example}%UNFOLD
...
# one sentence only:
fig_cap <- paste0("The empirical type I rate, over ",prty(nrep)," simulations, of a one-sided confidence bound for $\\psnr_{a}$ are shown ",
                  "for the hedged portfolio problem.")
