...
\end{equation*}
where $\vect{z} \sim\normlaw{\vzero,\eye}.$
Let $b$ be some constant we will tune later.
Then 
\begin{align*}
  \ssrsqopt - 2b\psnr[a]\ssropt 
  & \approx 
  \qiform{\pvsig}{\svmu} - 2b\tr{\pvmu}\minv{\pvsig}\svmu,\\
  &=
  \tr{\wrapParens{\svmu-2b\pvmu}}\minv{\pvsig}\svmu,\\
  &=
  \tr{\wrapParens{\svmu-b\pvmu}}\minv{\pvsig}\wrapParens{\svmu-b\pvmu} - b^2\tr{\pvmu}\minv{\pvsig}\pvmu,\\
  &=
  \tr{\wrapParens{\wrapParens{1-b}\pvmu + \oneby{\sqrt{\ssiz}}\chol{\pvsig}\vect{z}}} \minv{\pvsig}
  {\wrapParens{\wrapParens{1-b}\pvmu + \oneby{\sqrt{\ssiz}}\chol{\pvsig}\vect{z}}}
  - b^2\tr{\pvmu}\minv{\pvsig}\pvmu,\\
  &=
  \oneby{\ssiz}\tr{\wrapParens{\wrapParens{1-b}{\sqrt{\ssiz}}\pvmu + \chol{\pvsig}\vect{z}}} \minv{\pvsig}
  {\wrapParens{\wrapParens{1-b}{\sqrt{\ssiz}}\pvmu + \chol{\pvsig}\vect{z}}}
  - b^2\tr{\pvmu}\minv{\pvsig}\pvmu,\\
  &=
  \oneby{\ssiz}
  \chisqlaw{\nstrat, \ssiz\wrapParens{1-b}^2\psnrsqopt} - b^2\psnrsqopt,
\end{align*}%
where $\chisqlaw{\nstrat, \nctp}$ is a non-central chi-square random variable with $\nstrat$ degrees of freedom
and non-centrality parameter $\nctp$.

Now note that 
$$
c \le \ssiz \wrapParens{\ssrsqopt + b^2\psnrsqopt- 2b\psnr[a]\ssropt}
\Leftrightarrow 
\psnr[a] \le \oneby{2b}\wrapBracks{\ssropt + \frac{b^2\psnrsqopt}{\ssropt}} - \frac{c}{2b\ssiz\ssropt}.
$$
...
%\begin{align*}
  ...
  %+ \sum_{2 \le i \le \nstrat}z_i^2.
%\end{align*}
This condition will hold with probability \typeI if we set
\begin{equation}
  \cono[\typeI] = \chisqqnt{1-\typeI}{\nstrat, \ssiz\wrapParens{1-b}^2\psnrsqopt},
\end{equation}
where \chisqqnt{q}{\df, \nctp} is the $q$ quantile of the non-central chi-square distribution
with \df degrees of freedom and non-centrality parameter \nctp.
Because our bound depends on the unobserved \psnrsqopt, we optimistically set it to \ssrsqopt and arrive
at the confidence bound of
\begin{align}
  \psnr[a] 
  &\le \frac{1 + b^2}{2b} \ssropt - \frac{\chisqqnt{1-\typeI}{\nstrat, \ssiz\wrapParens{1-b}^2\psnrsqopt}}{2b\ssiz\ssropt},\\
  &= \ssropt + \frac{\ssiz\wrapParens{1-b}^2\ssrsqopt - {\chisqqnt{1-\typeI}{\nstrat, \ssiz\wrapParens{1-b}^2\psnrsqopt}}}{2b\ssiz\ssropt},
\end{align}
which holds approximately with probability \typeI.
In what follows we set $b = 1/2$, as it gives the closest to nominal coverage, but this is not well understood.
...
repsim <- function(nrep,zetas,n,p) {
  foo <- future_replicate(nrep,onesim(pzeta=zetas,n,p),future.seed=TRUE)
...
# 1e6 tkes 378 seconds with 20 cores.
nrep <- 1000000L

plan(multicore, gc = TRUE, workers = workers)
set.seed(2356)
#system.time({
results <- params %>%
  group_by(n,p) %>%
...
$\Sexpr{prty(nrep)}$ simulations of computing $\psnr[a]$ and \ssrsqopt, computing a
...
foores <- results %>%
  ...
  #mutate(cons = qchisq(1-typeI,df=p)/2)
  mutate(ncpval=n*omb*omb*pzeta*pzeta) %>%
  mutate(cons = (qchisq(1-typeI,df=p,ncp=ncpval) - ncpval) / (2*bval))
...
@
<<'badci_ci_plots',dependson=c('badci_collect_sims','plot_funcs'),fig.width=cfg_width,fig.height=cfg_height,out.width=cot_width,out.height=cot_height,fig.cap=fig_cap>>=
...
<<'feasible_ci_sims',dependson=c('badci_collect_sims','setup'),results='hide'>>=
typeI <- 0.05
qval <- 0.25
...
%
<<'feasible_ci_plots',dependson=c('feasible_ci_sims','plot_funcs'),fig.width=cfg_width,fig.height=cfg_height,out.width=cot_width,out.height=cot_height,fig.cap=fig_cap>>=
...
<<'hejci_plots',dependson=c('hejci_collect_sims','plot_funcs'),fig.width=cfg_width,fig.height=cfg_height,out.width=cot_width,out.height=cot_height,fig.cap=fig_cap>>=
fig_cap <- paste0("The empirical type I rate, over ",prty(nrep)," simulations, of a one-sided confidence bound for $\\psnr_{a}$ are shown ",
                  "for the hedged portfolio problem.  We set $\\nstrathej=\\nstrat/2$ and take $\\pvmu\\propto\\vone$. ",
                  "The SNR in the facet titles refers to $\\sqrt{\\Hejpsnrsqopt{\\hejG}}$. ")
...
<<'feasible_hejci_sims',dependson=c('hejci_collect_sims','setup'),results='hide'>>=
typeI <- 0.05
qval <- 0.25

Ffoo <- function(zval,fval,adj,df1,df2) {
	pf(fval,df1=df1,df2=df2,ncp=adj*zval^2)
}
qlim <- function(hejssrsqopt,subssrsqopt,ssiz,nstrat,nstrathej=nstrat/2,lvl=0.025) {
  df1 <- nstrat - nstrathej
  df2 <- ssiz - nstrat
  wrinkle <- (1 + (ssiz/ssiz-1)*subssrsqopt)
	fval <- (ssiz/(ssiz-1)) * (df2/df1) * hejssrsqopt / wrinkle
	ofunc <- function(z) { lvl - Ffoo(z,fval,adj=ssiz / wrinkle,df1=df1,df2=df2) }
	if (ofunc(0) > 0) { return(0) }
	zval <- uniroot(ofunc,lower=0,upper=20)$root
	return(zval)
}
# joint bounds
foo_cibound <- function(hejssrsqopt,subssrsqopt,ssiz,nstrat,nstrathej=nstrat/2,alpha=0.05,q=0.5) {
  # upper bound on psnropt
  ub <- qlim(hejssrsqopt,subssrsqopt,ssiz,nstrat,nstrathej,lvl=q*alpha)
  cons <- qchisq(1-(1-q)*typeI,df=nstrat-nstrathej,ncp=ssiz*ub*ub/4) - ssiz*ub*ub/4
  ssropt <- sqrt(hejssrsqopt)
  foobound <- ssropt - cons / (ssiz*ssropt)
}

# based on delta 2
# ignores q
baz_cibound <- function(hejssrsqopt,subssrsqopt,ssiz,nstrat,nstrathej=nstrat/2,alpha=0.05,...) {
  # make ub via delta_2
	df2 <- ssiz - nstrat
  wrinkle <- (1 + (ssiz/ssiz-1)*subssrsqopt)
	delta0 <- ((df2 - 2) / (ssiz-1)) * hejssrsqopt - ((nstrat-nstrathej)/ssiz) * wrinkle
	delta2 <- pmax(delta0,(2/(nstrat-nstrathej+2))*((df2-2)/(ssiz-1))*hejssrsqopt)
  ub <- sqrt(delta2)
  cons <- qchisq(1-typeI,df=nstrat-nstrathej,ncp=ssiz*ub*ub/4) - ssiz*ub*ub/4
  ssropt <- sqrt(hejssrsqopt)
  foobound <- ssropt - cons / (ssiz*ssropt)
}

ary_bound <- function(hejssrsqopts,subssrsqopts,ssiz,nstrat,nstrathej=nstrat/2,alpha=0.05,q=0.5,type=c('delta','worse')) {
	type <- match.arg(type)
  bdfnc <- switch(type,
                  delta=baz_cibound,
                  worse=foo_cibound)
  ylims <- future_mapply(FUN=bdfnc,hejssrsqopts,subssrsqopts,MoreArgs=list(ssiz=ssiz,nstrat=nstrat,nstrathej=nstrathej,alpha=alpha,q=q))
}
...
#system.time({
  ...
  hejnures <- hejresults %>%
    group_by(n,p) %>%
      mutate(ci_del = ary_bound(hejszeta^2,subssrsqopts=dzetasq,ssiz=first(n),nstrat=2*first(p),nstrathej=first(p),alpha=typeI,q=qval,type='delta'),
             ci_wrs = ary_bound(hejszeta^2,subssrsqopts=dzetasq,ssiz=first(n),nstrat=2*first(p),nstrathej=first(p),alpha=typeI,q=qval,type='worse')) %>%
    ungroup() %>%
    group_by(n,p,hejpzeta) %>%
      summarize(type1_del = mean(hejpsnra < ci_del),
                type1_wrs = mean(hejpsnra < ci_wrs)) %>%
    ungroup() 
  plan(sequential)
  #rm(hejresults)
#})
...
Once again, the $\delta_2$ plug-in estimator has coverage closer to the nominal $\Sexpr{typeI}$ rate,
and both bounds are anti-conservative when $\ssiz/\nstrat$ is not sufficiently large.
%
<<'feasible_hejci_plots',dependson=c('feasible_hejci_sims'),fig.width=cfg_width,fig.height=cfg_height,out.width=cot_width,out.height=cot_height,fig.cap=fig_cap>>=
fig_cap <- paste0("The empirical type I rate, over ",prty(nrep)," simulations, of two feasible one-sided confidence bounds for $\\psnr_{a}$ are shown ",
                  "for a nominal type I rate of $",typeI,"$. The daily returns are drawn from multivariate normal distribution with varying \\psnropt, \\ssiz, and \\nlatf. ",
                  "The $y$ axis is drawn in square root scale to show detail. ")
...
<<'example:73d3308a-7c58-44af-967d-25b279ab570c',results='asis',dependson=c('aqfb_data')>>=
suppressMessages({
	library(dplyr)
	library(xts)
	library(SharpeR)
...
# 19e0bbad#FOLDUP
snrfunc <- function(mp,svmu,svsig) { as.numeric(svmu %*% mp / sqrt(t(mp) %*% svsig %*% mp)) }

data(mff4)
sdat <- mff4[,! colnames(mff4) %in% c("RF")]
hejG <- diag(4)[which(grepl('Mkt',colnames(sdat))),,drop=FALSE]
...
svsig <- cov(sdat)
svmu  <- colMeans(sdat)
txtmp <- solve(svsig,svmu)

gsmu <- hejG %*% t(t(svmu))
gssig <- hejG %*% svsig %*% t(hejG)
gtxtmp <- t(hejG) %*% solve(gssig,gsmu)

porto <- txtmp - gtxtmp
cons <- as.numeric(Rbuj / sqrt(t(porto) %*% (svsig %*% porto)))
rownames(porto) <- rownames(svsig) 
sportwopt <- cons * porto
delsropt <- snrfunc(sportwopt,svmu,svsig)
unsropt <- snrfunc(txtmp,svmu,svsig)
Gsropt <- snrfunc(gtxtmp,svmu,svsig)

# save for later
example_19e0 <- list()
example_19e0$delsropt <- delsropt
example_19e0$unsropt <- unsropt
example_19e0$Gsropt <- Gsropt
example_19e0$ssiz <- ssiz
example_19e0$nstrat <- nstrat
example_19e0$nstrathej <- nstrathej
#UNFOLD
alp <- 0.05
# from bb7c5
Ffoo <- function(zval,fval,ssiz,df1,df2) {
...
# 8443aaf1#FOLDUP
# use this:
example_19e0 <- within(example_19e0,{ 
  alp <- alp
	df1 <- nstrat - nstrathej
	df2 <- ssiz - nstrat 
	denot <- (1 + ssiz * Gsropt^2 / (ssiz-1))
	#qlo <- denot * qlim(delsropt^2,ssiz,nstrat,df1=df1,df2=df2,lvl=1-alp/2)
	#qhi <- denot * qlim(delsropt^2,ssiz,nstrat,df1=df1,df2=df2,lvl=alp/2)
	qlo <- denot * qlim(delsropt^2,ssiz,nstrat,df1=df1,df2=df2,lvl=1-alp)
})
#UNFOLD
...
example_19e0 <- within(example_19e0,{ 
  # make ub via delta_2
	df2 <- ssiz - nstrat
  wrinkle <- (1 + (ssiz/ssiz-1)*Gsropt^2)
  # already computed delta2
	delta0 <- ((df2 - 2) / (ssiz-1)) * delsropt^2 - ((nstrat-nstrathej)/ssiz) * wrinkle
	delta2 <- pmax(delta0,(2/(nstrat-nstrathej+2))*((df2-2)/(ssiz-1))*delsropt^2)
  ub <- sqrt(delta2)
  cons <- qchisq(1-typeI,df=nstrat-nstrathej,ncp=ssiz*ub*ub/4) - ssiz*ub*ub/4
  ssropt <- delsropt
  hejlobound <- ssropt - cons / (ssiz*ssropt)
})
@
...
We compute
$$
\ssrsqopt=\Sexpr{example_19e0$unsropt^2}\,\moto{-1},\quad
\ssrsqoptG{\hejG}=\Sexpr{example_19e0$Gsropt^2}\,\moto{-1},\quad
\Hejssrsqopt{\hejG}=\Sexpr{example_19e0$delsropt^2}\,\moto{-1}.
$$
With $\ssiz=\Sexpr{example_19e0$ssiz}, \nstrat=\Sexpr{example_19e0$nstrat}$, we 
compute the three KRS estimators of \Hejpsnrsqopt{\hejG}, which all take the same value,
$\delta_0=\delta_1=\delta_2=\Sexpr{example_19e0$delta0}\moto{-1}.$
From this we compute a one-sided $\Sexpr{100*(1-alp)}\%$ confidence bound on 
%$\pql{\sportwoptHej{\hejG}}$ 
\psnr[a]
to be 
$\cointerval{\Sexpr{example_19e0$hejlobound}}{\infty}\,\moto{-\halff}$.
%\end{example}%UNFOLD
