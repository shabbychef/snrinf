...
\begin{alltt}
\hlcom{# https://stackoverflow.com/a/46377001/164611}
\hlstd{gitsha} \hlkwb{<-} \hlkwd{system}\hlstd{(}\hlstr{"git rev-parse HEAD"}\hlstd{,} \hlkwc{intern}\hlstd{=}\hlnum{TRUE}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
\providecommand{\gitHash}{8cdbf52ec0761be8e9e71389a5d02db97d5d49e7}
...
Again we assume \apxref{perfect_cov}.
...
$100000$ simulations of computing $\psnr[a]$ and \ssrsqopt, computing a
...
\includegraphics[width=0.975\textwidth,height=0.4875\textwidth]{figure/snrinfbadci_ci_plots-1} \caption[The empirical type I rate, over 100,000 simulations, of a one-sided confidence bound for $\psnr_{a}$ are shown for a nominal type I rate of $0.05$]{The empirical type I rate, over 100,000 simulations, of a one-sided confidence bound for $\psnr_{a}$ are shown for a nominal type I rate of $0.05$. The daily returns are drawn from multivariate normal distribution with varying \psnropt, \ssiz, and \nstrat. Type I rates are plotted versus $\ssiz/\nstrat$ to indicate the requisite aspect ratio to achieve near nominal coverage. }\label{fig:badci_ci_plots}
\end{figure}
...
%We use ``hedged'' to mean a portfolio with some fixed covariance (typically zero)
...
%The squared \txtSNR of the optimal portfolio is
%\begin{equation}
%\psnrsqopt
%= \qiform{\pvsig}{\pvmu} -
	%\qiform{\wrapParens{\qoform{\pvsig}{\hejG}}}{\wrapParens{\hejG\pvmu}}.
